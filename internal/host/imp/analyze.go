	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &tr) {
				...
				if salvaged := salvagePrefix(tr.Raw, seg, start); len(salvaged) > 0 {
					for i, f := range salvaged {
						...
						if werr := writeArtifact(w, analysisPath(ch), digest, art); werr != nil {
							return i, fmt.Errorf(i18n.T("imp.err.salvage_write"), ch, werr)
						}
					}
					w.writeFailure(FailureMeta{Stage: "analyze", Detail: fmt.Sprintf(i18n.T("imp.msg.batch_truncated"), start+1, end),
				...
				w.writeFailure(FailureMeta{Stage: "analyze", Detail: fmt.Sprintf(i18n.T("imp.msg.batch_truncated_no_prefix"), start+1, end),
