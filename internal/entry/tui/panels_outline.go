...
func renderOutlineList(snap host.UISnapshot, contentW int) string {
	...
	for _, e := range snap.Outline {
		...
		if snap.InProgressChapter == e.Chapter {
			line += lipgloss.NewStyle().Foreground(colorAccent).Italic(true).Render(i18n.T("outline.in_progress"))
		}
		b.WriteString(line)
		b.WriteString("\n")
	}
...
func renderDetailContent(snap host.UISnapshot, contentW int) string {
	...
	if len(snap.Outline) > 0 {
		outlineHeader := i18n.T("outline.title")
		if snap.Layered {
			outlineHeader = fmt.Sprintf(i18n.T("outline.title_layered"), snap.CurrentVolumeArc)
		}
		...
		if snap.Layered {
			if snap.NextVolumeTitle != "" {
				b.WriteString(compassStyle.Render(fmt.Sprintf(i18n.T("outline.next_volume"), snap.NextVolumeTitle)))
				b.WriteString("\n")
			}
			b.WriteString(compassStyle.Render(i18n.T("outline.more_chapters")))
			b.WriteString("\n")
			if snap.CompassDirection != "" {
				direction := fmt.Sprintf(i18n.T("outline.finale"), snap.CompassDirection)
				if snap.CompassScale != "" {
					direction += fmt.Sprintf(i18n.T("outline.scale_suffix"), snap.CompassScale)
				}
	...
	if len(snap.Characters) > 0 {
		b.WriteString(panelTitleStyle.Render(i18n.T("outline.characters")))
		b.WriteString("\n")
	...
	if snap.SupportingCount > 0 {
		b.WriteString(panelTitleStyle.Render(i18n.T("outline.supporting")))
		b.WriteString("\n")
		b.WriteString(cardContentStyle.Render(truncate(fmt.Sprintf(i18n.T("outline.appeared"), snap.SupportingCount), contentW)))
		b.WriteString("\n")
	...
	if snap.LastCommitSummary != "" {
		b.WriteString(cardTitleStyle.Render(i18n.T("outline.last_commit")))
		b.WriteString("\n")
	...
	if snap.LastReviewSummary != "" {
		b.WriteString(cardTitleStyle.Render(i18n.T("outline.last_review")))
		b.WriteString("\n")
	...
	if len(snap.RecentSummaries) > 0 {
		b.WriteString(cardTitleStyle.Render(i18n.T("outline.summary")))
		b.WriteString("\n")
