...
func (s *importState) refresh(contentW int) {
	...
	b.WriteString(titleStyle.Render(i18n.T("imp.title")))
	b.WriteString("\n\n")
	b.WriteString(dimStyle.Render(i18n.T("imp.source")))
	b.WriteString(s.source)
	b.WriteString("\n")
	b.WriteString(dimStyle.Render(i18n.T("imp.started")))
	b.WriteString(formatReportTime(s.startedAt))
	if !s.finishedAt.IsZero() {
		b.WriteString(dimStyle.Render(i18n.T("imp.finished")))
		b.WriteString(formatReportTime(s.finishedAt))
	}
	b.WriteString("\n\n")

	// 当前阶段行
	b.WriteString(mutedStyle.Render(i18n.T("imp.phase")))
	b.WriteString(stageStyle.Render(string(s.stage)))
	if s.total > 0 {
		b.WriteString(mutedStyle.Render(i18n.T("imp.progress")))
		if s.current > 0 {
	...
	b.WriteString(titleStyle.Render(i18n.T("imp.flow_log")))
	b.WriteString(" ")
	if s.totalLines > len(s.history) {
		b.WriteString(dimStyle.Render(fmt.Sprintf(i18n.T("imp.log_count_partial"), s.totalLines, len(s.history))))
	} else {
		b.WriteString(dimStyle.Render(fmt.Sprintf(i18n.T("imp.log_count"), s.totalLines)))
	}
	b.WriteString("\n")
	now := time.Now()
	...
	switch {
	case s.err != nil:
		b.WriteString(errStyle.Render(i18n.T("imp.failed")))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render(i18n.T("imp.esc_close")))
	case s.paused && s.stage == imp.StageAwaitingConfirmation:
		b.WriteString(okStyle.Render(i18n.T("imp.segment_ready")))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render(i18n.T("imp.segment_hint")))
	case s.paused:
		// 管线在等待裁定处停下，通道已关闭：按面板内提示操作后 Esc 关闭。
		b.WriteString(okStyle.Render(i18n.T("imp.paused")))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render(i18n.T("imp.paused_hint")))
	case s.done:
		b.WriteString(okStyle.Render(i18n.T("imp.done")))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render(i18n.T("imp.done_hint")))
	default:
		b.WriteString(dimStyle.Render(i18n.T("imp.esc_cancel")))
	}
...
func renderImportModal(width, height int, s *importState, frame int) string {
	...
	hint := i18n.T("imp.hint")
	switch {
	case s.paused && s.stage == imp.StageAwaitingConfirmation:
		hint = i18n.T("imp.hint_confirm")
	case running:
		hint = i18n.T("imp.hint_running")
	}
	...
	if running {
		...
		status := lipgloss.NewStyle().Foreground(colorMuted).
			Render(fmt.Sprintf(i18n.T("imp.running_status"), formatElapsed(time.Since(s.startedAt))))
		body = append([]string{star + status, ""}, body...)
	}
	modal := renderPaddedModalFrame(boxW, boxH, i18n.T("imp.modal_title"), hint, body)
...
func (m Model) confirmImportSegmentation() (tea.Model, tea.Cmd) {
	...
	if err != nil {
		m.applyEvent(host.Event{
			Time: time.Now(), Category: "ERROR", Summary: fmt.Sprintf(i18n.T("imp.err_confirm"), err.Error()), Level: "error",
		})
		return m, nil
	}
...
func parseImportArgs(args []string) (imp.Options, error) {
	...
	for i := range args {
		...
		case strings.HasPrefix(a, "--story="):
			...
			if v != "open" && v != "closed" {
				return imp.Options{}, fmt.Errorf(i18n.T("imp.err_story"), v)
			}
		...
		case strings.HasPrefix(a, "--guide="):
			...
			if g == "" {
				return imp.Options{}, errors.New(i18n.T("imp.err_guide"))
			}
			opts.Guidance = g
			return opts, nil
		case strings.HasPrefix(a, "--"):
			return imp.Options{}, fmt.Errorf(i18n.T("imp.err_unknown_opt"), a)
		default:
			if opts.SourcePath != "" {
				return imp.Options{}, fmt.Errorf(i18n.T("imp.err_dup_path"), a)
			}
			opts.SourcePath = a
		}
	}
