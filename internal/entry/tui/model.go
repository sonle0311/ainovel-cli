...
func (m *Model) inputHints() string {
	...
	suffix := limitHint + i18n.T("footer.copy_mode_suffix")
	if m.mode == modeNew {
		suffix = limitHint
	}
	if m.mouseOff && m.mode != modeNew {
		...
		return lipgloss.NewStyle().Foreground(colorAccent).Bold(true).
			Render(i18n.T("footer.copy_mode_active"))
	}
	if m.cocreate != nil {
		scrollHint := i18n.T("footer.scroll_chat")
		if m.cocreate.focusPrompt {
			scrollHint = i18n.T("footer.scroll_directive")
		}
		switch {
		case m.cocreate.awaiting:
			return dimStyle.Render(i18n.T("footer.cocreate_awaiting") + scrollHint + suffix)
		case m.cocreate.canStart():
			startLabel := i18n.T("footer.cocreate_start")
			if m.cocreate.stage {
				startLabel = i18n.T("footer.cocreate_apply")
			}
			return dimStyle.Render(i18n.T("footer.cocreate_send") + startLabel + " · " + i18n.T("footer.cocreate_exit") + scrollHint + suffix)
		default:
			return dimStyle.Render(i18n.T("footer.cocreate_send") + i18n.T("footer.cocreate_exit") + scrollHint + suffix)
		}
	}
	if m.mode == modeNew {
		if m.startupMode == startupModeQuick {
			return dimStyle.Render(i18n.T("footer.new_quick") + suffix)
		}
		return dimStyle.Render(i18n.T("footer.new_cocreate") + suffix)
	}
	switch m.snapshot.RuntimeState {
	case "pausing":
		return dimStyle.Render(i18n.T("footer.pausing") + suffix)
	case "paused":
		return dimStyle.Render(i18n.T("footer.paused") + suffix)
	}
	return dimStyle.Render(i18n.T("footer.running") + suffix)
}
...
func (m *Model) inputLimitHint() string {
	...
	return fmt.Sprintf(i18n.T("footer.char_limit"), used, limit)
}
...
func defaultSteerPlaceholder() string {
	return i18n.T("placeholder.steer")
}
...
func (m *Model) syncRuntimePlaceholder() {
	...
	if m.starting {
		m.textarea.Placeholder = i18n.T("placeholder.initializing")
		return
	}
	switch m.snapshot.RuntimeState {
	case "completed":
	...
	case "pausing":
		m.textarea.Placeholder = i18n.T("placeholder.pausing")
	case "paused":
		if m.snapshot.AdvanceMode == "review" && m.snapshot.Phase == "writing" {
			m.textarea.Placeholder = i18n.T("placeholder.review_wait")
		} else {
			m.textarea.Placeholder = i18n.T("placeholder.paused")
		}
	default:
		if !m.snapshot.IsRunning {
			if m.snapshot.AdvanceMode == "review" && m.snapshot.Phase == "writing" {
				...
				m.textarea.Placeholder = i18n.T("placeholder.interrupted")
			}
		} else {
