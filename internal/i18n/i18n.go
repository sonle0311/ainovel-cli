...
//   - 语言启动时设置一次，运行期可经 /lang 切换；current 为原子值，
//     TUI goroutine 写、host 事件 goroutine 读不会竞争。
...
// current 当前生效语言（原子读写，支持运行期 /lang 切换）。
...
func T(key string) string {
	if lang := Current(); lang != LangZH {
		if catalog := catalogFor(lang); catalog != nil {
